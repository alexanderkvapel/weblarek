...
`setData(fields: TField[] | TField): void` - сохранение данных в модели. Принимает объект, содержащий свойства: тип поля и значение поля или массив таких объектов.  
`clearData(): void` - очистка данных покупателя.  
`validateData(field: TField): string` - валидация данных. Принимает объект, содержащий свойства: тип поля и значение поля.
...
`postOrder(items: IProduct[]): Promise<IOrderResponse>` - отправляет на сервер данные о покупателе и выбранных товарах, полученные в параметре.
