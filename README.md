...
`getItemById(id: string): IProduct | null` - получение одного товара по его id.
...
`isItemInCart(item: IProduct): boolean` - проверка наличия товара в корзине по его id, полученного в параметр метода.
...
`setData(fields: TField[] | TField): void` - сохранение данных в модели.
`clearData(): void` - очистка данных покупателя.
`validateData(field: TField): string` - валидация данных.
